...
export const getApprovalTransaction = async (currentState: FrameState, yamlData: YamlData) => {
	if (!currentState.deploymentOption) {
		throw new Error('Deployment option is required to get approval transaction');
	}
...
export const getSubmissionTransaction = async (
	...
	if (!currentState.deploymentOption) {
		throw new Error('Deployment option is required to get submission transaction');
	}
