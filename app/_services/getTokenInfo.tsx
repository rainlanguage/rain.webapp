...
export const getTokenInfos = async (yaml: YamlData): Promise<TokenInfo[]> => {
  ...
  for (const [tokenName, token] of Object.entries(yaml.tokens)) {
    if (!isHex(token.address))
      throw new Error(
        `Token address for ${tokenName} is not a hex string: got ${token.address}`
      );

    if (!token.network)
      throw new Error(`Token ${tokenName} does not have a network`);
    ...
    if (res.some((r) => r.status !== "success")) {
      throw new Error(
        `Failed to fetch token info for ${tokenName}: ${res
          .map((r) => r.error)
          .join(", ")}`
      );
    }
